...
raw=read.table(file = "result.txt",header = FALSE,sep = " ",stringsAsFactors = FALSE,fill = TRUE)%>%
  ...
  select(-V17)
names(raw)=c("CellId",	"Freq1", "Freq2", "Vloc","Aloc","SepBF", "PrMix", "PrInt", "PrOut", "PrSing",	"WinModels", "Pval1",	"Pval2", "SampSizeA", "SampSizeB", "SampSizeAB","e")
model.names=c("Mixture","Intermediate","Outside","Single")
raw=raw%>%mutate(cid=paste(CellId,Freq1,Freq2,Vloc,Aloc,sep = "."))
...
ggplot(data = tri,mapping = aes(x = WinModel,color=WinProb,fill=WinProb))+
  ...
  scale_colour_grey(start = 0.9,end=0.3) + #if you use grey color
  scale_fill_grey(start = 0.9,end=0.3)
ggsave(filename = "plot.pdf",width = 12,height = 8)
...
#int and out
int=read.table("int_outresult.txt",stringsAsFactors = F,header = F)%>%
  mutate(e=e_)%>%
  mutate(cid=paste(V2,V3,V4,V5,sep = "."))

#mix
mix=read.table("mixresult.txt",stringsAsFactors = F,header = F)%>%
  mutate(e=e_,V7=e_+(1-2*e_)*V7,V8=e_+(1-2*e_)*V8))%>%
  mutate(cid=paste(V2,V3,V4,V5,sep = ".")) 
...
dfres=tri%>%
  ...
  mutate(cid=paste(CellId,PairId,AltFreq,AltPos,sep = "."))

ids=dfres%>%pull(cid)

pdf(height = 5, width = 10, file = "plot_pdf.pdf")
for (i in ids){
try({
int0=int%>%filter(cid==i,V1=="int")%>%.[,7:106]%>%t()
df1=data.frame(ind=seq(e_,1-e_,length.out = 100),y=int0,e=rep(e_,100),model=rep("int",100))
mix0=mix%>%filter(cid==i)%>%.[,c(7,8)]%>%t()
